...
class TextBox(InputBox):
    def __init__(self, name, color, rect, text='1000'):
    ...
    def draw(self):
        ...
        self.rect.w = max(surface.get_width() + 20, 50)  # Ensure the box is wide enough for the text
        ...
        if self.isActive:
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_BACKSPACE:
                    ...
                    print("Enter pressed, current text:", self.text)  # Handle the Enter key if needed
                elif event.unicode.isdigit():
                    self.text += event.unicode  # Add digit to the text
                self.draw()  # Redraw the text box with updated text
    ...
    def draw(self):
        ...
        if self.isActive:
            ...
            for i in range(self.DEFAUTL_OPTION+1, len(self.options)):
                ...
                options_color = white if i - 1 == self.active_option else grey
